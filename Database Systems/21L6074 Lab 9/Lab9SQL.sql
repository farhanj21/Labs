...
BEGIN
    ...
    SELECT @CGPA = AVG(GPA) FROM Registration WHERE RollNumber = @RollNumber;

    IF @CGPA < 2.5
    BEGIN
        PRINT 'The student can only enroll in subjects that they can improve.';
    END
    ELSE
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM Registration WHERE RollNumber = @RollNumber AND CourseID = @CourseID AND Semester = @Semester)
        BEGIN
        ...
        ELSE
        BEGIN
            PRINT 'The student is already registered for this course in the current semester.';
        END
    END
END;


--Student with CGPA < 2.5
EXEC RegisterStudent '1', 10, 'Spring2017', 'A', 0.0;

--Student with CGPA >= 2.5
EXEC RegisterStudent '2', 10, 'Spring2017', 'A', 0.0;

--Student with CGPA >= 2.5, but already registered for the course in the current semester
EXEC RegisterStudent '2', 10, 'Spring2017', 'A', 0.0;

--Student with CGPA >= 2.5 registering for a different course
EXEC RegisterStudent '2', 30, 'Spring2017', 'A', 0.0;
